from tests.bpi_helpers import SEG_VJ, order_body, pax_aux, product_item_for, search_body


def _search(client, segments=None):
    return client.post("/postBaggage", json=search_body(segments=segments)).json()
...
def test_order_happy_path(client):
    ...
    res = client.post("/orderCrossPostBaggage", json=body).json()
    assert res["status"] == "0"
...
def test_invalid_product_item_id_fails(client):
    ...
    res = client.post("/orderCrossPostBaggage", json=order_body("AUX-1", [pax_aux(SEG_VJ, item)])).json()
    assert res["status"] == "1"
...
def test_mismatched_weight_fails(client):
    ...
    res = client.post("/orderCrossPostBaggage", json=order_body("AUX-2", [pax_aux(SEG_VJ, tampered)])).json()
    assert res["status"] == "1"
    assert res["auxiliaryOrderNo"] is None
...
def test_idempotent_reorder(client):
    ...
    r1 = client.post("/orderCrossPostBaggage", json=order_body("DUP", [pax_aux(SEG_VJ, item20)])).json()
    r2 = client.post("/orderCrossPostBaggage", json=order_body("DUP", [pax_aux(SEG_VJ, item50)])).json()
